  ...
  return (
    <div className="px-4 space-y-6">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
                ...
                <tbody>
                  {rows.map((row) => (
                    <tr key={row.user_id} className="border-b border-oc-border/50">
                      ...
                      <td className="py-2">
                        <button onClick={() => void handleOpenDetail(row.user_id)} className="bg-oc-red text-white px-2 py-1 rounded">
                          Ver
                        </button>
                      </td>
                    </tr>
        ...
        <div className="bg-oc-metal p-4 rounded-lg border border-oc-red/20 space-y-4">
          ...
          {!detailLoading && detail && (
            <>
              <div className="bg-oc-dark rounded border border-oc-border p-3 text-sm grid grid-cols-2 gap-2">
                <div className="text-oc-muted">Nombre</div><div className="text-white">{detail.name}</div>
                <div className="text-oc-muted">Telefono</div><div className="text-white">{detail.phone || '-'}</div>
                <div className="text-oc-muted">Costo total</div><div className="text-white">{fmtMoney(activeCycle?.cost || 0)}</div>
                <div className="text-oc-muted">Total abonado</div><div className="text-white">{fmtMoney(activeCycle?.total_paid || 0)}</div>
                <div className="text-oc-muted">Saldo pendiente</div><div className="text-white">{fmtMoney(activeCycle?.pending_balance || 0)}</div>
                <div className="text-oc-muted">Adeudo historico</div><div className="text-white">{fmtMoney(detail.historical_pending_balance || 0)}</div>
                <div className="text-oc-muted">Adeudo total</div><div className="text-white">{fmtMoney(detail.total_pending_balance || 0)}</div>
                <div className="text-oc-muted">Estatus</div><div className="text-white">{statusLabel(activeCycle?.status || 'vencida')}</div>
              </div>
              ...
              <div className="bg-oc-dark rounded border border-oc-border p-3 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input className="bg-oc-metal border border-oc-border rounded px-2 py-2 text-white" value={cycleForm.membership_type} onChange={(e) => setCycleForm((p) => ({ ...p, membership_type: e.target.value }))} placeholder="Tipo de membresia" />
                  <input className="bg-oc-metal border border-oc-border rounded px-2 py-2 text-white" value={cycleForm.cost} onChange={(e) => setCycleForm((p) => ({ ...p, cost: e.target.value }))} placeholder="Costo" type="number" min={1} />
                  <input className="bg-oc-metal border border-oc-border rounded px-2 py-2 text-white" value={cycleForm.start_date} onChange={(e) => setCycleForm((p) => ({ ...p, start_date: e.target.value }))} type="date" />
                  <input className="bg-oc-metal border border-oc-border rounded px-2 py-2 text-white" value={cycleForm.end_date} onChange={(e) => setCycleForm((p) => ({ ...p, end_date: e.target.value }))} type="date" />
                </div>
                <select className="w-full bg-oc-metal border border-oc-border rounded px-2 py-2 text-white" value={cycleForm.manual_status} onChange={(e) => setCycleForm((p) => ({ ...p, manual_status: e.target.value }))}>
                ...
                <div className="flex gap-2">
                  <button disabled={busy} onClick={() => void handleSaveCycle(false)} className="bg-oc-panel text-white px-3 py-2 rounded">Guardar cambios</button>
                  <button disabled={busy} onClick={() => void handleSaveCycle(true)} className="bg-oc-red text-white px-3 py-2 rounded">Renovar ciclo</button>
                </div>
              </div>
              ...
              <div className="bg-oc-dark rounded border border-oc-border p-3 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input className="bg-oc-metal border border-oc-border rounded px-2 py-2 text-white" value={paymentForm.amount} onChange={(e) => setPaymentForm((p) => ({ ...p, amount: e.target.value }))} placeholder="Monto" type="number" min={1} />
                  <select className="bg-oc-metal border border-oc-border rounded px-2 py-2 text-white" value={paymentForm.payment_method} onChange={(e) => setPaymentForm((p) => ({ ...p, payment_method: e.target.value }))}>
                    <option value="efectivo">Efectivo</option>
                    <option value="transferencia">Transferencia</option>
                    <option value="tarjeta">Tarjeta</option>
                    <option value="otro">Otro</option>
                  </select>
                  <input className="bg-oc-metal border border-oc-border rounded px-2 py-2 text-white col-span-2" value={paymentForm.concept} onChange={(e) => setPaymentForm((p) => ({ ...p, concept: e.target.value }))} placeholder="Concepto o periodo cubierto" />
                  <input className="bg-oc-metal border border-oc-border rounded px-2 py-2 text-white col-span-2" value={paymentForm.observations} onChange={(e) => setPaymentForm((p) => ({ ...p, observations: e.target.value }))} placeholder="Observaciones (opcional)" />
                </div>
                <button disabled={busy || !activeCycle} onClick={() => void handleAddPayment()} className="bg-oc-red text-white px-3 py-2 rounded">Registrar pago</button>
                <div className="max-h-36 overflow-auto text-sm space-y-1">
                  {filteredPayments.map((payment) => (
                    <div key={payment.id} className="bg-oc-metal p-2 rounded border border-oc-border">
                      <div className="text-white">{fmtMoney(payment.amount)} - {payment.payment_method}</div>
                      <div className="text-oc-muted text-xs">{fmtDate(payment.payment_date)} - {payment.created_by_name || 'sistema'}</div>
              ...
              <div className="bg-oc-dark rounded border border-oc-border p-3 space-y-2">
                ...
                <textarea className="w-full bg-oc-metal border border-oc-border rounded px-2 py-2 text-white" rows={3} value={noteText} onChange={(e) => setNoteText(e.target.value)} placeholder="Escribe una nota interna del cliente" />
                <button disabled={busy} onClick={() => void handleAddNote()} className="bg-oc-red text-white px-3 py-2 rounded">Agregar nota</button>
                <div className="max-h-36 overflow-auto text-sm space-y-1">
                  {detail.notes.map((note) => (
                    <div key={note.id} className="bg-oc-metal p-2 rounded border border-oc-border">
                      ...
                      <div className="text-oc-muted text-xs">{fmtDate(note.created_at)} - {note.created_by_name || 'sistema'}</div>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="text-sm space-y-1 max-h-40 overflow-auto">
                  {detail.cycles_history.map((cycle) => (
                    <div key={cycle.id} className="bg-oc-metal p-2 rounded border border-oc-border text-oc-light/90">
                      {cycle.membership_type} | {fmtDate(cycle.start_date)} - {fmtDate(cycle.end_date)} | {fmtMoney(cycle.cost)} | {statusLabel(cycle.status)}
                    </div>
                  ))}
                </div>
              </div>
